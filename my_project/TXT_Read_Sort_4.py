...
class TXTtoXLSXApp:
    def __init__(self, root):
        self.root = root
        self.root.title("TXT/TXT4 → XLSX / CSV Merger + Sort")

        self.txt_files = []
        self.target_file = None

        tk.Label(root, text="Wybierz pliki TXT/TXT4").pack(pady=5)
        tk.Button(root, text="Wybierz TXT/TXT4", command=self.load_txt).pack()
    ...
    def load_txt(self):
        files = filedialog.askopenfilenames(
            filetypes=[("Pliki tekstowe", "*.txt *.txt4")]
        )
        for file in files:
    ...
    def merge(self):
        if not self.txt_files or not self.target_file:
            messagebox.showerror("Błąd", "Wybierz pliki TXT/TXT4 i plik docelowy")
            return
